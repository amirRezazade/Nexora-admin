...
import {
  mapProduct, mapCategory, mapCustomer, mapOrder, mapOrderItem,
  mapCoupon, mapReview, mapNotification, mapInventory, productToRow, stockStatus,
} from './maps';
...
async function allOrders() {
  const [{ data: orders, error }, items] = await Promise.all([
...
function decorateCategory(c, products, categories) {
  ...
  const descendantCount = children.reduce(
    (s, ch) => s + products.filter((p) => p.categoryId === ch.id).length,
    productCount
  );
...
async function getProducts(params) {
  ...
  const rawCats = params.getAll('category').flatMap((v) => String(v).split(',')).filter((v) => v && v !== 'all');
  if (rawCats.length) {
...
async function createCategory(body) {
  ...
  const id = `cat-${body.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 20)}`;
  const row = {
    id,
    name: body.name.trim(),
...
async function getReviews(params) {
  ...
  const rating = params.getAll('rating').flatMap((v) => v.split(',')).filter(Boolean);
...
async function getNotifications(params) {
  ...
  const counts = all.reduce((acc, n) => { acc[n.category] = (acc[n.category] || 0) + 1; return acc; }, {});
  counts.all = all.length;
...
async function getDashboard(params) {
  ...
  return {
    kpis: {
      ...
      customers: {
        current: customers.length, change: buyers.change, previous: buyers.previous,
        newInPeriod: newCustomers, spark: series.slice(-14).map((d) => d.orders),
      },
      products: {
        current: products.filter((p) => p.status === 'active').length, change: 0,
    ...
    activity: orders.slice(0, 7).map((o) => ({
      id: `act-${o.id}`, type: 'order', actor: o.customerName,
      message: `placed order #${o.id}`, at: o.placedAt, href: `/orders/${o.id}`, amount: o.total,
    })),
  };
}
...
async function getAnalytics(params) {
  ...
  const [orders, products, customers, categories] = await Promise.all([
    allOrders(), allProducts(), allCustomers(), allCategories(),
  ]);
  const revenue = compare(days, revenueOf, orders);
  const orderCount = compare(days, (l) => l.length, orders);
  ...
  inWindow.forEach((o) => { hourly[new Date(o.placedAt).getUTCHours()].orders += 1; });
  ...
  return {
    ...
    geography: Object.values(byCountry).sort((a, b) => b.revenue - a.revenue).slice(0, 8),
    hourly,
  };
}
...
async function getSearch(params) {
  ...
  return {
    products: products
      ...
      .slice(0, 5)
      .map((p) => ({
        id: p.id, type: 'product', title: p.name, titleFa: p.nameFa, hint: `${p.sku} · $${p.price.toFixed(2)}`,
        href: `/products/${p.id}`, status: p.stockStatus, image: p.image,
      })),
    orders: orders
      .filter((o) => o.id.toLowerCase().includes(q) || (o.customerName || '').toLowerCase().includes(q))
      .slice(0, 5)
      .map((o) => ({
        id: o.id, type: 'order', title: `#${o.id}`,
        hint: `${o.customerName} · $${o.total.toFixed(2)}`, href: `/orders/${o.id}`, status: o.status,
      })),
    customers: customers
      .filter((c) => c.name.toLowerCase().includes(q) || c.email.toLowerCase().includes(q))
      .slice(0, 5)
      .map((c) => ({
        id: c.id, type: 'customer', title: c.name, hint: c.email, href: `/customers/${c.id}`, status: c.status,
      })),
    pages: PAGES.filter((p) => p.title.toLowerCase().includes(q)).slice(0, 4).map((p) => ({ ...p, type: 'page' })),
  };
}
