...
function readStoredLocale() {
  ...
  try {
    const l = window.localStorage.getItem('nova-locale');
    return l === 'fa' ? 'fa' : 'en';
  } catch {
    return 'en';
  }
...
export const dateShort = (d) =>
  new Date(d).toLocaleDateString(intlLocale(), { month: 'short', day: 'numeric', year: 'numeric' });

export const dateMedium = (d) =>
  new Date(d).toLocaleDateString(intlLocale(), { month: 'short', day: 'numeric', year: 'numeric' });
...
export const timeOnly = (d) =>
  new Date(d).toLocaleTimeString(intlLocale(), { hour: 'numeric', minute: '2-digit' });
