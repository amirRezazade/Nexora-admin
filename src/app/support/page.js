...
import {
  BookOpen, MessageSquare, Mail, ExternalLink, ChevronDown, LifeBuoy, Keyboard,
} from 'lucide-react';
...
export default function SupportPage() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    const errs = {};
    if (!values.subject.trim()) errs.subject = 'Add a short subject.';
    if (!values.message.trim()) errs.message = 'Describe what you need help with.';
    else if (values.message.trim().length < 20) errs.message = 'Please add a little more detail (at least 20 characters).';
    setErrors(errs);
            ...
            <CardBody>
              <form onSubmit={submit} noValidate className="flex flex-col gap-4">
                <Input
                  ...
                  onChange={(e) => { setValues((v) => ({ ...v, subject: e.target.value })); setErrors((p) => ({ ...p, subject: undefined })); }}
                ...
                <Textarea
                  ...
                  onChange={(e) => { setValues((v) => ({ ...v, message: e.target.value })); setErrors((p) => ({ ...p, message: undefined })); }}
              ...
              <dl className="divide-y divide-line">
                {SHORTCUTS.map(([key, label]) => (
                  <div key={key} className="flex items-center justify-between gap-4 py-2">
                    ...
                    <dd>
                      <kbd className="rounded border border-line bg-surface-2 px-1.5 py-0.5 font-mono text-caption font-semibold text-ink">
                        {key}
                      </kbd>
                    </dd>
                  </div>
