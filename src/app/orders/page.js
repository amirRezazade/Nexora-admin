...
export default function OrdersPage() {
  ...
  const chips = useMemo(() => {
    ...
    filters.payment.forEach((v) =>
      out.push({ key: 'payment', value: v, label: t('ordersPage.colPayment'), display: t(`status.${v}`) })
    );
        ...
        {selected.length > 0 ? (
          <BulkBar count={selected.length} itemLabel="order" onClear={() => dispatch(clearSelection())}>
            <Button
              size="sm"
              variant="secondary"
              onClick={() => exportOrders(items.filter((i) => selected.includes(i.id)), t('ordersPage.itemLabel'))}
            >
              {t('common.export')}
            </Button>
            <Dropdown
