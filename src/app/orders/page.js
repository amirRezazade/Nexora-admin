...
import { Search, SlidersHorizontal, Download, X, Eye, Printer, MoreHorizontal, ShoppingCart, Truck, PackageCheck, XCircle } from "lucide-react";
import { cn, currency, dateShort, number, relativeTime, titleCase } from "@/lib/format";
import { useDebouncedValue } from "@/lib/hooks";
import { downloadCSV } from "@/lib/export";
import { fetchOrders, setFilter, setStatusTab, setPage, setPageSize, setSort, clearFilters, toggleRow, toggleAllRows, clearSelection, updateOrderStatus } from "@/store/slices/ordersSlice";
...
export default function OrdersPage() {
  ...
  const { items, status, error, filters, sort, page, pageSize, total, totalPages, counts, selected } = useSelector((s) => s.orders);

  const [searchInput, setSearchInput] = useState(() => searchParams.get("q") || filters.q);
  ...
  useEffect(() => {
    if (urlQ) {
      setSearchInput(urlQ);
      dispatch(setFilter({ key: "q", value: urlQ }));
    }
  }, [urlQ, dispatch]);
  ...
  const chips = useMemo(() => {
    const out = [];
    filters.payment.forEach((v) => out.push({ key: "payment", value: v, label: t("ordersPage.colPayment"), display: t(`status.${v}`) }));
    ...
    <div className="flex flex-col gap-5">
      ...
      <Card className="flex flex-col">
        <div className="px-4 sm:px-5">
          <Tabs tabs={tabs} value={activeTab} ariaLabel="Filter orders by status" onChange={(v) => dispatch(setStatusTab(v))} />
        </div>
        ...
        {selected.length > 0 ? (
              ...
              {({ close }) =>
                ["processing", "shipped", "delivered"].map((s) => (
                  <MenuItem
                    ...
                    onClick={async () => {
                      close();
                      await Promise.all(items.filter((i) => selected.includes(i.id)).map((o) => dispatch(updateOrderStatus({ id: o.id, status: s }))));
                      dispatch(clearSelection());
                      dispatch(toast.success(t("toast.orderBulk", { n: selected.length, status: t(`status.${s}`) })));
                    }}
                  >
                    {t("ordersPage.markAs", { status: t(`status.${s}`) })}
                  </MenuItem>
                ))
              }
            </Dropdown>
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-line px-4 py-3 sm:px-5">
            <div className="relative min-w-0 flex-1 sm:max-w-xs">
              <Input type="search" icon={Search} placeholder={t("ordersPage.search")} aria-label={t("ordersPage.search")} value={searchInput} onChange={(e) => setSearchInput(e.target.value)} className="h-8" />
              {searchInput && (
                <button type="button" onClick={() => setSearchInput("")} aria-label={t("common.clear")} className="absolute right-2 top-1/2 -translate-y-1/2 rounded p-0.5 text-ink-3 hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand">
            ...
            <Button size="sm" variant="secondary" icon={SlidersHorizontal} onClick={() => setFiltersOpen(true)}>
              {t("common.filters")}
              {chips.length > 0 && <span className="ml-0.5 rounded-pill bg-brand px-1.5 text-[10px] font-bold leading-4 text-white">{chips.length}</span>}
            </Button>
            <Button size="sm" variant="ghost" icon={Download} className="ms-auto" onClick={() => exportOrders(items, t("ordersPage.itemLabel"))}>
              {t("common.export")}
            </Button>
          </div>
        )}
        ...
        {status === "failed" ? (
          <ErrorState title={t("ordersPage.loadError")} description={t("common.tryAgain")} onRetry={() => dispatch(fetchOrders())} />
          ...
          <EmptyState
            ...
            description={hasFilters ? t("ordersPage.noneFoundHint") : t("ordersPage.emptyHint")}
            action={
              hasFilters && (
                <Button
                  variant="secondary"
              ...
              <THead>
                <tr>
                  <TH width="44px" className={cn(stickyLeft, "bg-surface-2")}>
                    <Checkbox checked={allSelected} indeterminate={!allSelected && someSelected} onChange={() => dispatch(toggleAllRows())} aria-label="Select all orders on this page" />
                  </TH>
                  <TH sortable sortKey="id" currentSort={sort} onSort={(k) => dispatch(setSort(k))}>
                    {t("ordersPage.colId")}
                  </TH>
                  <TH sortable sortKey="customerName" currentSort={sort} onSort={(k) => dispatch(setSort(k))} className="min-w-[200px]">
                    {t("ordersPage.colCustomer")}
                  </TH>
                  <TH sortable sortKey="placedAt" currentSort={sort} onSort={(k) => dispatch(setSort(k))}>
                    {t("ordersPage.colDate")}
                  </TH>
                  <TH align="right" sortable sortKey="itemCount" currentSort={sort} onSort={(k) => dispatch(setSort(k))}>
                    {t("ordersPage.colItems")}
                  </TH>
                  <TH align="right" sortable sortKey="total" currentSort={sort} onSort={(k) => dispatch(setSort(k))}>
                  ...
                  return (
                    <TR key={o.id} selected={isSelected}>
                      <TD className={stickyLeft}>
                        <Checkbox checked={isSelected} onChange={() => dispatch(toggleRow(o.id))} aria-label={`Select order ${o.id}`} />
                      </TD>
                      <TD>
                        <Link href={`/orders/${o.id}`} className="font-mono text-caption font-semibold text-ink underline-offset-4 transition-colors hover:text-brand-text hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand">
                          #{o.id}
                        </Link>
                      </TD>
                      <TD>
                        <div className="flex items-center gap-2.5">
                          ...
                          <div className="min-w-0">
                            <Link href={`/customers/${o.customerId}`} className="block truncate text-body-sm font-medium text-ink underline-offset-4 hover:text-brand-text hover:underline">
                              {o.customerName}
                            </Link>
                      ...
                      <TD align="right" className={stickyRight}>
                        <Dropdown menuLabel={`Actions for order ${o.id}`} trigger={<IconButton icon={MoreHorizontal} size="sm" label={`Actions for order ${o.id}`} />}>
                          {({ close }) => (
                            <>
                              <MenuItem
                                icon={Eye}
                              ...
                              <MenuItem
                                ...
                                onClick={() => {
                                  close();
                                  dispatch(toast.info(t("toast.printDemo")));
                                }}
                              >
        ...
        <Pagination page={page} totalPages={totalPages} total={total} pageSize={pageSize} itemLabel={t("ordersPage.itemLabel")} onPageChange={(p) => dispatch(setPage(p))} onPageSizeChange={(s) => dispatch(setPageSize(s))} />
      </Card>
      ...
      >
        <div className="flex flex-col gap-7">
              ...
              {Object.entries(PAYMENT_STATUS).map(([value, cfg]) => (
                <Checkbox
                  ...
                  onChange={() =>
                    setDraft((d) => ({
                      ...d,
                      payment: d.payment.includes(value) ? d.payment.filter((v) => v !== value) : [...d.payment, value],
                    }))
                  }
                />
              ))}
          ...
          <fieldset className="border-0 p-0">
            ...
            <Input type="number" min="0" label={t("ordersPage.minTotal")} prefix="$" className="pl-7" placeholder="0.00" value={draft.minTotal} onChange={(e) => setDraft((d) => ({ ...d, minTotal: e.target.value }))} />
          </fieldset>
        </div>
      </Drawer>
    </div>
