...
import {
  ...
  CreditCard, MapPin, User, Receipt, ShoppingCart, CheckCircle2, Clock, RotateCcw,
} from 'lucide-react';
    ...
    return (
      <Card>
        <EmptyState
          ...
          action={<Button as={Link} href="/orders" variant="primary">Back to Orders</Button>}
        />
      </Card>
    );
  }
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-h1 text-ink">{t('ordersPage.colId')} #{o.id}</h1>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Dropdown
            ...
            trigger={<IconButton icon={MoreHorizontal} label="More order actions" variant="secondary" size="lg" />}
          >
            {({ close }) => (
              <>
                  ...
                  {!isLast && (
                    <span
                      ...
                      className={cn(
                        ...
                        o.timeline[i + 1]?.done ? 'bg-brand' : 'bg-line'
                      )}
                    />
                  )}
                </li>
          ...
          <Timeline className="md:hidden">
            {o.timeline.map((step, i) => (
              <TimelineItem
                ...
                last={i === o.timeline.length - 1}
                title={step.label}
                meta={step.at ? dateShort(step.at) : '—'}
                description={step.done ? 'Completed' : 'Pending'}
              />
            ))}
          </Timeline>
        </CardBody>
              ...
              <Timeline>
                {[...o.timeline].filter((s) => s.done).reverse().map((step, i, arr) => (
                  <TimelineItem
                    ...
                    description={
                      step.key === 'placed'
                        ? `${o.customerName} placed the order`
                        : step.key === 'payment'
                        ? `${currency(o.total)} charged to ${o.paymentMethod}`
                        : step.key === 'shipped' && o.trackingNumber
                        ? `Dispatched via ${o.shippingMethod} — ${o.trackingNumber}`
                        : 'Updated by the fulfilment team'
                    }
              ...
              <dl className="space-y-2 text-body-sm">
                ...
                <div className="flex justify-between gap-3">
                  ...
                  <dd className="tabular-nums text-ink">
                    {o.shipping === 0 ? <span className="text-success-text">Free</span> : currency(o.shipping)}
                  </dd>
                </div>
                <div className="flex justify-between gap-3">
          ...
          <Card>
            ...
            <CardBody>
              ...
              <p className="mt-3 border-t border-line pt-3 text-caption leading-relaxed text-ink-3">
                Payment details are illustrative. No real transactions are processed in this demo.
              </p>
            </CardBody>
          </Card>
