...
import { ArrowLeft, Printer, MoreHorizontal, Mail, Copy, XCircle, Truck, PackageCheck, CreditCard, MapPin, Receipt, ShoppingCart, CheckCircle2, Clock, RotateCcw } from "lucide-react";
...
export default function OrderDetailsPage() {
  ...
  const itemsLabel = o.itemCount === 1 ? t("orderDetail.itemsCountOne") : t("orderDetail.itemsCount", { n: number(o.itemCount) });
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          <IconButton icon={ArrowLeft} label={t("orderDetail.back")} variant="secondary" onClick={() => router.push("/orders")} className="mt-1 shrink-0" />
          <div className="min-w-0">
            <h1 className="text-h1 text-ink">{t("orderDetail.title", { id: o.id })}</h1>
            <p className="mt-1 text-body text-ink-2">{t("orderDetail.placedAt", { date: dateTime(o.placedAt), relative: relativeTime(o.placedAt) })}</p>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Dropdown menuLabel={t("orderDetail.moreActions")} trigger={<IconButton icon={MoreHorizontal} label={t("orderDetail.moreActions")} variant="secondary" size="lg" />}>
            {({ close }) => (
              <>
                <MenuItem
                  icon={Mail}
                  onClick={() => {
                    close();
                    dispatch(toast.success(t("toast.confirmResent"), t("toast.confirmResentHint", { email: o.customerEmail })));
                  }}
                >
      ...
      <Card>
        <CardHeader title={t("orderDetail.status")} description={o.trackingNumber ? t("orderDetail.tracking", { n: o.trackingNumber }) : t("orderDetail.notDispatched")} />
        <CardBody>
              ...
              return (
                <li key={step.key} className="flex flex-1 items-start last:flex-none">
                  <div className="flex min-w-0 flex-col items-center gap-2 px-1 text-center">
                    ...
                    <span className={cn("text-caption font-medium", step.done ? "text-ink" : "text-ink-3")}>{stepTitle(t, step)}</span>
                  ...
                  {!isLast && <span aria-hidden className={cn("mt-[18px] h-0.5 min-w-6 flex-1 rounded-pill", timeline[i + 1]?.done ? "bg-brand" : "bg-line")} />}
                </li>
              );
            })}
          </ol>
          ...
          <Timeline className="md:hidden">
            {timeline.map((step, i) => (
              <TimelineItem key={step.key} icon={STEP_ICONS[step.key] || Clock} tone={step.tone === "danger" ? "danger" : step.tone === "warning" ? "warning" : step.done ? "brand" : "muted"} last={i === timeline.length - 1} title={stepTitle(t, step)} meta={step.at ? dateShort(step.at) : "—"} description={step.done ? t("orderDetail.completed") : t("status.pending")} />
            ))}
          </Timeline>
        </CardBody>
      </Card>
                ...
                {[...o.timeline]
                  ...
                  .map((step, i, arr) => (
                    <TimelineItem
                      ...
                      description={step.key === "placed" ? t("orderDetail.placedBy", { name: o.customerName }) : step.key === "payment" ? t("orderDetail.chargedTo", { amount: currency(o.total), method: payMethod }) : step.key === "shipped" && o.trackingNumber ? t("orderDetail.dispatchedVia", { method: shipMethod, tracking: o.trackingNumber }) : t("orderDetail.updatedByTeam")}
        ...
        <aside className="flex flex-col gap-4">
            ...
            <CardBody>
              <dl className="space-y-2 text-body-sm">
                ...
                <div className="flex justify-between gap-3">
                  ...
                  <dd className="tabular-nums text-ink">{o.shipping === 0 ? <span className="text-success-text">{t("orderDetail.free")}</span> : currency(o.shipping)}</dd>
                </div>
                <div className="flex justify-between gap-3">
          ...
          <Card>
            ...
            <CardBody>
              ...
              <p className="mt-3 border-t border-line pt-3 text-caption leading-relaxed text-ink-3">{t("orderDetail.paymentHint")}</p>
            </CardBody>
          </Card>
