      ...
      {status === 'failed' ? (
        ...
        <>
            ...
            {loading || !k ? (
              ...
              <>
                <StatCard
                  ...
                  comparison={COMPARISON_LABEL[range]}
                ...
                <StatCard
                  ...
                  comparison={COMPARISON_LABEL[range]}
                  spark={k.customers.spark}
                  icon={Users}
                  tone="success"
                  hint={`${number(k.customers.newInPeriod)} new this period`}
                />
                <StatCard
                  label="Products"
                  ...
                  icon={Package}
                  tone="warning"
                  hint={`${k.products.lowStock} low stock · ${k.products.outOfStock} out of stock`}
                  footer={
                    (k.products.lowStock > 0 || k.products.outOfStock > 0) && (
                      <Link
                        href="/inventory?status=low_stock"
                        className="mt-1 inline-block text-caption font-medium text-brand-text underline-offset-4 hover:underline"
                      >
                        Review inventory
                      </Link>
                    )
            ...
            <Card>
              <CardHeader
                ...
                description={
                  loading || !k
                    ? t('common.loading')
                    : t('dashboard.earned', { amount: currency(k.revenue.current, { decimals: 0 }), period: t(`dashboard.last${range === '12m' ? '12' : range.replace('d', '')}`) })
                }
                action={<SegmentedControl options={RANGES} value={range} onChange={setRange} ariaLabel={t('dashboard.rangeAria')} />}
              />
              <div className="px-2 py-5 sm:px-4">
                {loading ? <SkeletonChart /> : <RevenueChart data={data.series} granularity={data.granularity} />}
              </div>
            </Card>
            ...
            <Card>
              <CardHeader
                ...
                description={
                  loading || !k
                    ? t('common.loading')
                    : t('dashboard.ordersUnits', { orders: number(k.orders.current), units: number(k.units.current) })
                }
              />
              <div className="px-2 py-5 sm:px-4">
          ...
          <section className="grid grid-cols-1 gap-4 xl:grid-cols-2">
            ...
            <TopProducts products={data?.topProducts} loading={loading} periodLabel={PERIOD_LABEL[range]} />
          </section>
