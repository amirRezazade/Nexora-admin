...
import {
  Search, Plus, FolderTree, Pencil, Trash2, MoreHorizontal, X, CornerDownRight, Package,
} from 'lucide-react';
import { cn, number, slugify, localized } from '@/lib/format';
import { useI18n } from '@/i18n/I18nProvider';
import { useDebouncedValue } from '@/lib/hooks';
import {
  fetchCategories, setFilter, setSort, setPage, createCategory, updateCategory, deleteCategory,
  ...
  return (
    ...
    >
      <form id="category-dialog-form" onSubmit={submit} className="flex flex-col gap-4 pb-2">
        ...
        <Input
          label={t('form.nameFa')}
          placeholder="مثلاً کت و کاپشن"
          value={values.nameFa}
          onChange={(e) => setValues((v) => ({ ...v, nameFa: e.target.value }))}
          dir="rtl"
          lang="fa"
        />
        <Input
        ...
        <div className="grid grid-cols-2 gap-4">
          <Select
            ...
            options={parents.filter((p) => p.id !== category?.id).map((p) => ({ value: p.id, label: localized(p, 'name', locale) }))}
        ...
        <Textarea
          ...
          rows={3}
          placeholder="A short summary shown on the collection page."
        ...
        <Textarea
          ...
          rows={3}
          placeholder="خلاصهٔ فارسی این دسته"
          value={values.descriptionFa}
          onChange={(e) => setValues((v) => ({ ...v, descriptionFa: e.target.value }))}
          dir="rtl"
          lang="fa"
        />
      </form>
...
export default function CategoriesPage() {
  const { t, locale } = useI18n();
  const dispatch = useDispatch();
  const { items, all, status, filters, sort, page, total, totalPages, pageSize } =
      ...
      <Card className="flex flex-col">
        <div className="flex flex-wrap items-center gap-2 border-b border-line px-4 py-3 sm:px-5">
          <div className="relative min-w-0 flex-1 sm:max-w-xs">
            <Input
              ...
              aria-label={t('categoriesPage.searchAria')}
              value={searchInput}
        ...
        {status === 'failed' ? (
          ...
          <EmptyState
            ...
            description={
              hasFilters
                ? t('categoriesPage.noneFoundHint')
                : t('categoriesPage.emptyHint')
            }
            action={
              <Button variant="primary" icon={Plus} onClick={() => setDialog({ open: true, category: null })}>
                {t('form.addCategory')}
            ...
            <Table>
              ...
              <THead>
                <tr>
                  ...
                  <TH width="60px" align="right"><span className="sr-only">{t('common.actions')}</span></TH>
                </tr>
              </THead>
              <TBody>
                {items.map((c) => (
                  <TR key={c.id}>
                    <TD>
                      <div className="flex items-start gap-2.5">
                        {c.parentId && (
                          <CornerDownRight aria-hidden className="mt-0.5 h-3.5 w-3.5 shrink-0 text-ink-3" />
                        )}
                        <div className="min-w-0">
                          ...
                          {c.parentId ? (
                            <p className="text-caption text-ink-3">{t('categoriesPage.inParent', { name: parentName(c.parentId) })}</p>
                          ) : c.description ? (
                            <p className="hidden truncate text-caption text-ink-3 sm:block">{c.description}</p>
                          ) : null}
                        </div>
                      </div>
                    </TD>
                    ...
                    <TD align="right" numeric>
                      <Link
                        ...
                        className="font-medium text-ink underline-offset-4 hover:text-brand-text hover:underline"
                      >
                        {number(c.productCount)}
                      </Link>
                      {c.childCount > 0 && c.descendantCount !== c.productCount && (
                        <span className="block text-caption text-ink-3">{t('categoriesPage.withSubs', { n: number(c.descendantCount) })}</span>
                      )}
                    </TD>
                      ...
                      >
                        {({ close }) => (
                          <>
                            ...
                            <MenuItem icon={Pencil} onClick={() => { close(); setDialog({ open: true, category: c }); }}>
                              {t('common.edit')}
                            </MenuItem>
                            <MenuSeparator />
                            <MenuItem
                              icon={Trash2}
                              destructive
                              onClick={() => { close(); setConfirmDelete(c); }}
                            >
                              {t('common.delete')}
                            </MenuItem>
                          </>
        ...
        <Pagination
          ...
          total={total}
          pageSize={pageSize}
          itemLabel={t('categoriesPage.itemLabel')}
