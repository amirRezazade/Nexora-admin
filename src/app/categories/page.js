...
function CategoryDialog({ open, onClose, category, parents }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setErrors(typeof err === 'object' ? err : { name: 'Something went wrong.' });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <Card className="flex flex-col">
        ...
        {status === 'failed' ? (
          <ErrorState title="We couldn’t load your categories." description="Please try again." onRetry={() => dispatch(fetchCategories())} />
          ...
          <TableWrap className={cn(status === 'loading' && 'pointer-events-none opacity-60')}>
            <Table>
              <caption className="sr-only">Product categories, including their parent and product counts.</caption>
              <THead>
                <tr>
                  ...
                  <TR key={c.id}>
                    <TD>
                      <div className="flex items-start gap-2.5">
                        ...
                        <div className="min-w-0">
                          ...
                          {c.parentId ? (
                            <p className="text-caption text-ink-3">in {parentName(c.parentId)}</p>
                          ) : c.description ? (
                            <p className="truncate text-caption text-ink-3">{c.description}</p>
                          ) : null}
                        </div>
                      </div>
                    </TD>
                    ...
                    <TD align="right" numeric>
                      ...
                      {c.childCount > 0 && c.descendantCount !== c.productCount && (
                        <span className="block text-caption text-ink-3">{number(c.descendantCount)} with subs</span>
                      )}
                    </TD>
                    <TD>
