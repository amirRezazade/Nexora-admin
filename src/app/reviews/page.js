...
import {
  Search, Star, Check, EyeOff, Trash2, MoreHorizontal, X, Eye, MessageSquare,
} from 'lucide-react';
...
export default function ReviewsPage() {
  ...
  const load = async () => {
    ...
    try {
      const res = await api(
        `/api/reviews${qs({ q: debounced, status: tab === 'all' ? '' : tab, rating: ratingFilter, page, pageSize: 10 })}`
      );
  ...
  return (
    <div className="flex flex-col gap-5">
      <PageHeader
        ...
        description={t('reviewsPage.description')}
      >
        <div className="mt-3 flex items-center gap-3">
          <Rating value={state.average || 0} size="md" />
          <span className="text-body-sm text-ink-2">
            {t('reviewsPage.average', { n: number(state.counts?.all || 0) })}
          </span>
        </div>
      </PageHeader>
      ...
      <Card className="flex flex-col">
        <div className="px-4 sm:px-5">
          <Tabs tabs={tabs} value={tab} ariaLabel="Filter reviews by moderation status" onChange={(v) => { setTab(v); setPage(1); }} />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-line px-4 py-3 sm:px-5">
          ...
          />
          {hasFilters && (
            <Button size="sm" variant="ghost" onClick={() => { setQuery(''); setRatingFilter(''); }}>
              {t('common.clear')}
            </Button>
          )}
        </div>
        ...
        {state.status === 'failed' ? (
          ...
          <EmptyState
            ...
            description={
              hasFilters || tab !== 'all'
                ? t('reviewsPage.noneFoundHint')
                : t('reviewsPage.emptyHint')
            }
            action={
              (hasFilters || tab !== 'all') && (
                <Button variant="secondary" onClick={() => { setQuery(''); setRatingFilter(''); setTab('all'); }}>
            ...
            <Table>
              ...
              <THead>
                <tr>
                  ...
                  <TH className="hidden sm:table-cell">{t('reviewsPage.colStatus')}</TH>
                  ...
                  <TR key={r.id}>
                    ...
                    <TD className="hidden sm:table-cell">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          ...
                          {r.verified && <Badge tone="success" size="sm">{t('reviewsPage.verified')}</Badge>}
                        </div>
                      </div>
                    </TD>
                    <TD className="hidden md:table-cell">
                      <Link
                        ...
                        className="block max-w-[180px] truncate text-body-sm text-ink-2 underline-offset-4 hover:text-brand-text hover:underline"
                      >
                        {r.productName}
                      </Link>
                    </TD>
                      ...
                      <div className="flex items-center justify-end gap-1">
                        {r.status === 'pending' && (
                          <IconButton
                            icon={Check}
                            size="sm"
                            label={`Publish review from ${r.customerName}`}
                            className="hidden text-success-text hover:bg-success-soft sm:inline-flex"
                            onClick={() => moderate(r, 'published')}
                          />
                        )}
                        <Dropdown
                          menuLabel={`Actions for review from ${r.customerName}`}
                          trigger={<IconButton icon={MoreHorizontal} size="sm" label={`Actions for review from ${r.customerName}`} />}
                        >
                          {({ close }) => (
                            <>
                              ...
                              <MenuSeparator />
                              {r.status !== 'published' && (
                                <MenuItem icon={Check} onClick={() => { close(); moderate(r, 'published'); }}>
                                  {t('reviewsPage.publish')}
                                </MenuItem>
                              )}
                              {r.status !== 'hidden' && (
                                <MenuItem icon={EyeOff} onClick={() => { close(); moderate(r, 'hidden'); }}>
                                  {t('reviewsPage.hide')}
                                </MenuItem>
                              )}
                              <MenuSeparator />
                              <MenuItem icon={Trash2} destructive onClick={() => { close(); setConfirmDelete(r); }}>
                                {t('common.delete')}
                              </MenuItem>
                            </>
                          )}
        ...
        <Pagination
          ...
          total={state.total}
          pageSize={10}
          itemLabel={t('reviewsPage.itemLabel')}
      ...
      <ConfirmDialog
        ...
        confirmLabel={t('reviewsPage.deleteConfirm')}
