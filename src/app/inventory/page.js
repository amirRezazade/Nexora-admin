...
import { Search, Download, X, Boxes, AlertTriangle, PackageX, DollarSign, Pencil, Check } from "lucide-react";
import { cn, currency, dateShort, number, relativeTime, localized } from "@/lib/format";
import { useI18n } from "@/i18n/I18nProvider";
import { useDebouncedValue } from "@/lib/hooks";
import { downloadCSV } from "@/lib/export";
import { fetchInventory, setFilter, setSort, setPage, setPageSize, clearFilters, adjustStock } from "@/store/slices/inventorySlice";
...
function StockEditor({ row }) {
  ...
  if (!editing) {
    return (
      <button type="button" onClick={() => setEditing(true)} aria-label={`Edit available stock for ${row.name}, currently ${row.available}`} className={cn("group/edit inline-flex items-center gap-1.5 rounded-control px-1.5 py-0.5 font-medium tabular-nums transition-colors hover:bg-surface-3", "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand", row.status === "out_of_stock" ? "text-danger-text" : row.status === "low_stock" ? "text-warning-text" : "text-ink")}>
...
export default function InventoryPage() {
  ...
  const { items, status, filters, sort, page, pageSize, total, totalPages, summary } = useSelector((s) => s.inventory);
        ...
        {loading && !summary ? (
          ...
          <>
            ...
            <StatCard
              ...
              footer={
                summary?.lowStock > 0 && (
                  <button type="button" onClick={() => dispatch(setFilter({ key: "status", value: ["low_stock"] }))} className="mt-1 text-caption font-medium text-brand-text underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand">
                    Show these items
                  </button>
                )
              }
      ...
      <Card className="flex flex-col">
        <div className="flex flex-wrap items-center gap-2 border-b border-line px-4 py-3 sm:px-5">
          <div className="relative min-w-0 flex-1 sm:max-w-xs">
            <Input type="search" icon={Search} placeholder={t("inventoryPage.search")} aria-label={t("inventoryPage.search")} value={searchInput} onChange={(e) => setSearchInput(e.target.value)} className="h-8" />
            {searchInput && (
          ...
          <Select size="sm" aria-label="Filter by stock status" options={[{ value: "", label: t("inventoryPage.allStock") }, ...Object.entries(STOCK_STATUS).map(([value]) => ({ value, label: t(`status.${value}`) }))]} value={filters.status[0] || ""} onChange={(e) => dispatch(setFilter({ key: "status", value: e.target.value ? [e.target.value] : [] }))} className="w-40" />

          <Select size="sm" aria-label="Filter by category" options={[{ value: "", label: t("inventoryPage.allCategories") }, ...categories.map((c) => ({ value: c.id, label: c.name }))]} value={filters.category[0] || ""} onChange={(e) => dispatch(setFilter({ key: "category", value: e.target.value ? [e.target.value] : [] }))} className="w-40" />
            ...
            <Table>
              ...
              <THead>
                <tr>
                  ...
                  <TH align="right" sortable sortKey="available" currentSort={sort} onSort={(k) => dispatch(setSort(k))}>
                    {t("inventoryPage.colAvailable")}
                  </TH>
                  <TH align="right" className="hidden md:table-cell">
                    {t("inventoryPage.colReserved")}
                  </TH>
              ...
              <TBody>
                {items.map((row) => (
                  <TR key={row.id}>
                    <TD className={stickyLeft}>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <Link href={`/products/${row.productId}`} className="block truncate text-body-sm font-semibold text-ink underline-offset-4 transition-colors hover:text-brand-text hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand">
                    ...
                    <TD align="right" numeric muted className="hidden md:table-cell">
                      {number(row.reserved)}
                    </TD>
                    <TD align="right" numeric muted className="hidden lg:table-cell">
                      {number(row.threshold)}
                    </TD>
                    <TD align="right" numeric strong className="hidden xl:table-cell">
                      {currency(row.value, { decimals: 0 })}
                    </TD>
                    <TD>
        ...
        <Pagination page={page} totalPages={totalPages} total={total} pageSize={pageSize} itemLabel={t("inventoryPage.itemLabel")} onPageChange={(p) => dispatch(setPage(p))} onPageSizeChange={(s) => dispatch(setPageSize(s))} />
      </Card>
    </div>
  );
}
