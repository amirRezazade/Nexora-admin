...
import { DollarSign, ShoppingCart, Receipt, Users, Repeat, Package, Download, Globe, Clock } from "lucide-react";
    ...
    <div className="flex flex-col gap-6">
      ...
      >
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <Select size="sm" aria-label="Date range" options={RANGES.map((r) => ({ value: r.value, label: t(RANGE_NAME_KEYS[r.value]) }))} value={range} onChange={(e) => setRange(e.target.value)} className="w-44" />
      ...
      {status === "failed" ? (
        <Card>
          <ErrorState title={t("analyticsPage.loadError")} description="The reporting service didn’t respond. Please try again." onRetry={load} />
        </Card>
      ) : (
        <>
          {/* Primary metrics */}
          <section aria-label="Headline metrics" className="grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-3">
            {loading || !k ? (
              ...
              <>
                <StatCard label={t("analyticsPage.revenue")} value={currency(k.revenue.current, { decimals: 0 })} change={compare ? k.revenue.change : null} comparison={compare ? COMPARISON[range] : null} spark={data.series.slice(-14).map((d) => d.revenue)} icon={DollarSign} tone="brand" hint={compare ? `Previously ${currency(k.revenue.previous, { decimals: 0 })}` : null} />
                <StatCard label={t("analyticsPage.orders")} value={number(k.orders.current)} change={compare ? k.orders.change : null} comparison={compare ? COMPARISON[range] : null} spark={data.series.slice(-14).map((d) => d.orders)} icon={ShoppingCart} tone="info" hint={compare ? `Previously ${number(k.orders.previous)}` : null} />
                <StatCard label={t("analyticsPage.aov")} value={currency(k.aov.current)} change={compare ? k.aov.change : null} comparison={compare ? COMPARISON[range] : null} icon={Receipt} tone="success" hint={compare ? `Previously ${currency(k.aov.previous)}` : null} />
                <StatCard label={t("analyticsPage.buyers")} value={number(k.customers.current)} change={compare ? k.customers.change : null} comparison={compare ? COMPARISON[range] : null} icon={Users} tone="warning" hint="Unique buyers in this period" />
                <StatCard label={t("analyticsPage.repeat")} value={`${k.repeatRate.current}%`} icon={Repeat} tone="info" hint="Buyers with more than one order" />
                <StatCard label={t("analyticsPage.units")} value={number(k.units.current)} change={compare ? k.units.change : null} comparison={compare ? COMPARISON[range] : null} spark={data.series.slice(-14).map((d) => d.units)} icon={Package} tone="brand" />
              </>
            )}
          </section>
          ...
          <Card>
            <CardHeader title={t("analyticsPage.salesTrend")} description={loading ? t("common.loading") : t(RANGE_NAME_KEYS[range])} />
            <div className="px-2 py-5 sm:px-4">{loading ? <SkeletonChart height="h-80" /> : <RevenueChart data={data.series} granularity={data.granularity} height={320} />}</div>
          </Card>
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
            ...
            <Card>
              ...
              <CardBody>{loading ? <SkeletonChart height="h-56" /> : <CategoryChart data={data.categories} />}</CardBody>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
            <Card>
              ...
              <div className="px-2 py-5 sm:px-4">{loading ? <SkeletonChart /> : <CustomerGrowthChart data={data.customerGrowth} />}</div>
            </Card>

            <Card>
              <CardHeader title={t("analyticsPage.performance")} description="Top products by revenue" />
              <div className="px-2 py-5 sm:px-4">{loading ? <SkeletonChart height="h-72" /> : <ProductPerformanceChart data={data.products} />}</div>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)]">
            ...
            <div className="flex flex-col gap-4">
                      ...
                      return (
                        <li key={g.country} className="flex items-center gap-3 py-1.5">
                          ...
                          <span className="h-1.5 flex-1 overflow-hidden rounded-pill bg-surface-3" aria-hidden>
                            <span className="block h-full rounded-pill bg-brand transition-[width] duration-500" style={{ width: `${(g.revenue / max) * 100}%` }} />
                          </span>
                          <span className="w-16 shrink-0 text-right text-body-sm font-medium tabular-nums text-ink">{currencyCompact(g.revenue)}</span>
                          <span className="hidden w-10 shrink-0 text-right text-caption tabular-nums text-ink-3 sm:block">{g.orders}</span>
                        </li>
                      );
                    })}
                  </ul>
              ...
              <Card>
                <CardHeader title={t("analyticsPage.byHour")} icon={Clock} description={peakHour ? `Peak trading at ${String(peakHour.hour).padStart(2, "0")}:00 UTC` : "Loading…"} />
                <CardBody>
                  {loading ? (
                    ...
                    <div className="flex h-24 items-end gap-[3px]" role="img" aria-label="Order volume by hour of day">
                      {data.hourly.map((h) => {
                        ...
                        return <span key={h.hour} title={`${String(h.hour).padStart(2, "0")}:00 — ${h.orders} orders`} className={cn("flex-1 rounded-t-[3px] transition-colors", isPeak ? "bg-brand" : "bg-brand/25 hover:bg-brand/45")} style={{ height: `${Math.max(4, (h.orders / max) * 100)}%` }} />;
                      })}
                    </div>
                  )}
