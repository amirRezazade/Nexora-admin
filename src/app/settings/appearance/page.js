...
function ThemePreview({ scheme }) {
  ...
  const c = isDark ? { canvas: "#0c0e11", surface: "#16191e", line: "#2a2f37", ink: "#9ea5b0", bar: "#3a4049" } : { canvas: "#f6f7f9", surface: "#ffffff", line: "#e5e7eb", ink: "#828995", bar: "#d1d5db" };

  return (
    <span aria-hidden className="block overflow-hidden rounded-control border" style={{ backgroundColor: c.canvas, borderColor: c.line }}>
  ...
  return (
    <span aria-hidden className="relative block h-[72px] overflow-hidden rounded-control border border-line">
      ...
      <span className="absolute inset-y-0 right-0 w-1/2 overflow-hidden">
        <span className="block w-[200%] -translate-x-1/2">
            ...
            options={[
              {
                ...
                preview: (
                  <span aria-hidden className="flex flex-col gap-2 rounded-control border border-line bg-surface-2 p-2.5">
                    {[1, 2, 3].map((i) => (
                      <span key={i} className="h-1.5 rounded-pill bg-line-strong" />
                    ))}
                  </span>
                ),
              },
              {
                ...
                preview: (
                  <span aria-hidden className="flex flex-col gap-1 rounded-control border border-line bg-surface-2 p-2.5">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <span key={i} className="h-1 rounded-pill bg-line-strong" />
                    ))}
                  </span>
                ),
              },
      ...
      <Card>
        ...
        <CardBody>
          <div className="flex flex-wrap items-center gap-4">
            ...
            <p className="max-w-xs text-caption leading-relaxed text-ink-3">{t("settingsPages.accentBody")}</p>
          </div>
        </CardBody>
      </Card>
    </SettingsLayout>
