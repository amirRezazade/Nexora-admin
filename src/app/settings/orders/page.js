  ...
  return (
    <SettingsLayout title={t('settings.orders')} description={t('settingsPages.ordersHint')} dirty={dirty} onSave={save}>
      <Card>
        ...
        <CardBody className="flex flex-col gap-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Input
              label={t('settingsPages.prefix')}
              value={form.prefix}
              onChange={patch('prefix')}
              hint={t('settingsPages.prefixHint')}
              className="font-mono"
            />
            <Input
              label={t('settingsPages.nextNumber')}
              type="number"
              value={form.nextNumber}
              onChange={patch('nextNumber')}
              className="font-mono"
            />
          </div>
          <p className="rounded-card bg-surface-2 px-4 py-3 text-body-sm text-ink-2">
            {t('settingsPages.nextPreview', { id: `${form.prefix}${form.nextNumber}` })}
          </p>
        </CardBody>
      </Card>
      ...
      <Card>
        ...
        <CardBody className="flex flex-col gap-5">
          <Switch
            ...
            label={t('settingsPages.autoFulfil')}
            description={t('settingsPages.autoFulfilHint')}
          />
      ...
      <Card>
        ...
        <CardBody className="flex flex-col gap-5">
          <Switch
            ...
            label={t('settingsPages.autoCancel')}
            description={t('settingsPages.autoCancelHint')}
          />
          {form.autoCancel && (
            <div className="border-t border-line pt-5">
              <Select
      ...
      <Card>
        ...
        <CardBody className="flex flex-col gap-5">
          <Switch
            checked={form.notifyPlaced}
            onChange={track('notifyPlaced')}
            label={t('settingsPages.notifyPlaced')}
            description={t('settingsPages.notifyPlacedHint')}
          />
          <div className="border-t border-line pt-5">
            <Switch
              checked={form.notifyShipped}
              onChange={track('notifyShipped')}
              label={t('settingsPages.notifyShipped')}
              description={t('settingsPages.notifyShippedHint')}
            />
          </div>
          <div className="border-t border-line pt-5">
            <Switch
              checked={form.notifyDelivered}
              onChange={track('notifyDelivered')}
              label={t('settingsPages.notifyDelivered')}
              description={t('settingsPages.notifyDeliveredHint')}
            />
          </div>
        </CardBody>
      </Card>
