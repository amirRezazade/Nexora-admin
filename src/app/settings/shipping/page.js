...
const REGION_KEYS = ["settingsPages.regionEU", "settingsPages.regionGB", "settingsPages.regionCH", "settingsPages.regionNO", "settingsPages.regionUS", "settingsPages.regionCA", "settingsPages.regionAU", "settingsPages.regionJP"];
...
export default function ShippingSettingsPage() {
  ...
  return (
    <SettingsLayout title={t("settingsPages.shippingTitle")} description={t("settingsPages.shippingHint")} dirty={dirty} onSave={save}>
      <Card>
        <CardHeader
        ...
        <CardBody className="flex flex-col gap-5">
          ...
          {form.freeEnabled && (
            <div className="border-t border-line pt-5">
              <Input label={t("settingsPages.freeThreshold")} type="number" min="0" prefix="$" className="ps-7 sm:max-w-xs" value={form.freeThreshold} onChange={patch("freeThreshold")} hint={t("settingsPages.freeOver", { amount: currency(parseFloat(form.freeThreshold) || 0, { decimals: 0 }) })} />
            </div>
          )}
        </CardBody>
      </Card>
      ...
      <Card>
        ...
        <CardBody>
          ...
          <Button size="sm" variant="secondary" icon={Plus} className="mt-4">
            {t("settingsPages.addRegion")}
          </Button>
        </CardBody>
      </Card>
    </SettingsLayout>
