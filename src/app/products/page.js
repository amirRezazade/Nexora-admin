...
import { Plus, Search, SlidersHorizontal, Columns3, Download, Upload, X, Check } from "lucide-react";
import { cn, titleCase, localized } from "@/lib/format";
import { useI18n } from "@/i18n/I18nProvider";
import { useDebouncedValue } from "@/lib/hooks";
import { downloadCSV } from "@/lib/export";
import { fetchProducts, setFilter, setFilters, setPage, setPageSize, clearAllFilters, toggleColumn, resetColumns, clearSelection, bulkUpdateProducts, deleteProduct, duplicateProduct, DEFAULT_COLUMNS } from "@/store/slices/productsSlice";
...
const COLUMN_KEYS = {
  product: "products.colProduct",
  sku: "products.colSku",
  category: "products.colCategory",
  price: "products.colPrice",
  stock: "products.colStock",
  status: "products.colStatus",
  updated: "products.colUpdated",
  supplier: "products.colSupplier",
};
...
export default function ProductsPage() {
  ...
  const { filters, sort, page, pageSize, total, totalPages, selected, columns, summary, items } = useSelector((s) => s.products);
  ...
  useEffect(() => {
    const patch = {};
    if (urlCategory) patch.category = urlCategory.split(",").filter(Boolean);
    if (urlQ) {
      patch.q = urlQ;
      setSearchInput(urlQ);
    }
    if (Object.keys(patch).length) dispatch(setFilters(patch));
  }, [urlCategory, urlQ, dispatch]);
  ...
  const chips = useMemo(() => {
    const out = [];
    filters.category.forEach((v) => out.push({ key: "category", value: v, label: t("filters.category"), display: catName(v) }));
    filters.status.forEach((v) => out.push({ key: "status", value: v, label: t("filters.status"), display: t(`status.${v}`) }));
    filters.stock.forEach((v) => out.push({ key: "stock", value: v, label: t("filters.stock"), display: t(`status.${v}`) }));
    ...
    if (filters.addedAfter) out.push({ key: "addedAfter", value: filters.addedAfter, label: t("filters.addedAfter"), display: filters.addedAfter });
    return out;
  }, [filters, catName, t]);
  ...
  return (
    <div className="flex flex-col gap-5">
      <PageHeader
        ...
        actions={
          <>
            <Button variant="secondary" icon={Upload} className="hidden sm:inline-flex" onClick={() => dispatch(toast.info(t("toast.importDemo"), t("toast.importDemoHint")))}>
              {t("common.import")}
            </Button>
      ...
      <Card className="flex flex-col">
        ...
        <div className="px-4 sm:px-5">
          <Tabs tabs={statusTabs} value={activeTab} ariaLabel="Filter products by status" onChange={(v) => dispatch(setFilter({ key: "status", value: v === "all" ? [] : [v] }))} />
        </div>
        ...
        {selected.length > 0 ? (
          <BulkBar count={selected.length} itemLabel="product" onClear={() => dispatch(clearSelection())}>
              ...
              {({ close }) =>
                Object.entries(PRODUCT_STATUS).map(([value, cfg]) => (
                  <MenuItem
                    ...
                    onClick={() => {
                      close();
                      runBulk("status", value, t("toast.productBulkStatus", { n: selected.length, status: t(`status.${value}`) }));
                    }}
                  >
                    {t(`status.${value}`)}
                  </MenuItem>
              ...
              {({ close }) =>
                categories.map((c) => (
                  <MenuItem
                    ...
                    onClick={() => {
                      close();
                      runBulk("category", c.id, t("toast.productBulkMoved", { n: selected.length, name: localized(c, "name", locale) }));
                    }}
                  >
            ...
            <Button size="sm" variant="secondary" onClick={() => runBulk("archive", null, t("toast.productBulkArchived", { n: selected.length }))}>
              {t("products.archive")}
            </Button>
          </BulkBar>
        ) : (
          <div className="flex flex-wrap items-center gap-2 border-b border-line px-4 py-3 sm:px-5">
            <div className="relative min-w-0 flex-1 sm:max-w-xs">
              <Input type="search" icon={Search} placeholder={t("products.search")} aria-label={t("products.searchAria")} value={searchInput} onChange={(e) => setSearchInput(e.target.value)} className="h-8" />
              {searchInput && (
            ...
            >
              <div className="p-1">
                <div className="flex items-center justify-between px-2.5 pb-1 pt-2">
                  <span className="text-micro uppercase tracking-wide text-ink-3">{t("products.visibleColumns")}</span>
                  <button type="button" onClick={() => dispatch(resetColumns())} className="rounded text-caption font-medium text-brand-text hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand">
                    {t("common.reset")}
                  </button>
                </div>
                <ul className="py-1">
                  {Object.keys(DEFAULT_COLUMNS).map((key) => {
                    ...
                    return (
                      <li key={key}>
                        <label className={cn("flex cursor-pointer items-center gap-2.5 rounded-[7px] px-2.5 py-1.5 transition-colors hover:bg-surface-2", isLast && "cursor-not-allowed opacity-50")}>
                          <Checkbox checked={columns[key]} disabled={isLast} onChange={() => dispatch(toggleColumn(key))} />
                          <span className="text-body-sm text-ink">{t(COLUMN_KEYS[key])}</span>
                        </label>
                      </li>
                    );
                  })}
            ...
            <div className="ms-auto hidden items-center gap-2 lg:flex">
              <Button size="sm" variant="ghost" icon={Download} onClick={() => handleExport(items, t("products.itemLabel"))}>
                {t("common.export")}
              </Button>
        ...
        {chips.length > 0 && <FilterChips chips={chips} onRemove={removeChip} onClearAll={clearEverything} className="border-b border-line px-4 py-2.5 sm:px-5" />}
        ...
        <Pagination page={page} totalPages={totalPages} total={total} pageSize={pageSize} itemLabel={t("products.itemLabel")} onPageChange={(p) => dispatch(setPage(p))} onPageSizeChange={(s) => dispatch(setPageSize(s))} />
      </Card>

      <ProductFilters open={filtersOpen} onClose={() => setFiltersOpen(false)} categories={categories} />

      <ConfirmDialog open={Boolean(confirmDelete)} onClose={() => setConfirmDelete(null)} onConfirm={doDelete} loading={deleting} title={t("products.deleteTitle")} message={confirmDelete ? t("products.deleteMessage", { name: confirmDelete.name }) : ""} confirmLabel={t("products.deleteConfirm")} />
    </div>
  );
}
