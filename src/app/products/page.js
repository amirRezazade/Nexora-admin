...
import { fetchProducts, setFilter, setPage, setPageSize, clearAllFilters, toggleColumn, resetColumns, clearSelection, bulkUpdateProducts, deleteProduct, duplicateProduct, DEFAULT_COLUMNS } from "@/store/slices/productsSlice";
...
export default function ProductsPage() {
  ...
  const handleExport = (rows, label) => {
    downloadCSV(`nova-products-${new Date().toISOString().slice(0, 10)}.csv`, rows, [
  ...
  const runBulk = async (action, value, message) => {
    try {
      ...
      dispatch(toast.error("We couldn’t update those products.", "Please try again."));
    }
  };
  ...
  const doDelete = async () => {
    ...
    try {
      ...
      dispatch(toast.success("Product deleted", `“${confirmDelete.name}” was removed from your catalog.`));
  ...
  return (
    <div className="flex flex-col gap-5">
      <PageHeader
        ...
        actions={
          <>
            <Button variant="secondary" icon={Upload} className="hidden sm:inline-flex" onClick={() => dispatch(toast.info("Import is not available in this demo.", "Connect a data source to enable CSV import."))}>
              {t("common.import")}
            </Button>
      ...
      <Card className="flex flex-col">
        ...
        {selected.length > 0 ? (
          <BulkBar count={selected.length} itemLabel="product" onClear={() => dispatch(clearSelection())}>
              ...
              {({ close }) =>
                Object.entries(PRODUCT_STATUS).map(([value, cfg]) => (
                  <MenuItem
                    ...
                    onClick={() => {
                      close();
                      runBulk("status", value, `${selected.length} product${selected.length === 1 ? "" : "s"} set to ${cfg.label.toLowerCase()}.`);
                    }}
                  >
                    {t(`status.${value}`)}
                  </MenuItem>
              ...
              {({ close }) =>
                categories.map((c) => (
                  <MenuItem
                    ...
                    onClick={() => {
                      close();
                      runBulk("category", c.id, `Moved ${selected.length} product${selected.length === 1 ? "" : "s"} to ${c.name}.`);
                    }}
                  >
            ...
            <Button size="sm" variant="secondary" onClick={() => runBulk("archive", null, `${selected.length} product${selected.length === 1 ? "" : "s"} archived.`)}>
              {t("products.archive")}
            </Button>
          </BulkBar>
        ) : (
        ...
        <ProductTable
          ...
          onDuplicate={async (p) => {
            try {
              ...
              dispatch(toast.success("Product duplicated", `“${copy.name}” was created as a draft.`));
            } catch {
              dispatch(toast.error("We couldn’t duplicate the product.", "Please try again."));
            }
          }}
          onArchive={async (p) => {
            ...
            try {
              ...
              dispatch(toast.success(next === "archived" ? "Product archived" : "Product restored", `“${p.name}” is now ${next}.`));
            } catch {
              dispatch(toast.error("We couldn’t update the product.", "Please try again."));
            }
          }}
