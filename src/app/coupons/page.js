...
import { Search, Plus, Ticket, Pencil, Trash2, MoreHorizontal, X, Copy, Percent, DollarSign, Truck, Power } from "lucide-react";
...
function CouponDialog({ open, onClose, coupon, onSaved }) {
  ...
  return (
    <Modal
      ...
      description={editing ? t("couponsPage.editHint") : t("couponsPage.createHint")}
      size="lg"
      footer={
        <>
          ...
          <Button variant="primary" onClick={submit} loading={saving}>
            {saving ? t("common.saving") : editing ? t("common.save") : t("couponsPage.create")}
          </Button>
        </>
      }
    >
      <form onSubmit={submit} noValidate className="flex flex-col gap-4 pb-2">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Input
            ...
            hint={!errors.code ? t("couponsPage.codeHint") : undefined}
          />
          <Select
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {!isShipping && (
            <Input
              label={values.type === "percentage" ? t("couponsPage.discountPercent") : t("couponsPage.discountAmount")}
              required
              type="number"
              min="0"
          ...
          <Input label={t("couponsPage.minOrder")} type="number" min="0" step="0.01" prefix="$" className="pl-7" placeholder="0.00" value={values.minOrder} onChange={set("minOrder")} hint={t("couponsPage.minHint")} />
          {isShipping && (
            <div className="flex items-end">
              <p className="rounded-card bg-info-soft px-3 py-2 text-caption leading-relaxed text-info-text">{t("couponsPage.shippingNote")}</p>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <Input label={t("couponsPage.limit")} type="number" min="0" placeholder={t("couponsPage.unlimited")} value={values.limit} onChange={set("limit")} hint={t("couponsPage.limitHint")} />
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Select
            ...
            options={[
              ...
              { value: "categories", label: t("couponsPage.specificCategories") },
              { value: "products", label: t("couponsPage.specificProducts") },
            ]}
        ...
        <Textarea label={t("couponsPage.note")} rows={2} placeholder={t("couponsPage.note")} value={values.description} onChange={set("description")} />
      </form>
    </Modal>
  );
}
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <Card className="flex flex-col">
                ...
                {state.data.map((c) => {
                  ...
                  return (
                    <TR key={c.id}>
                        ...
                        <div className="flex flex-col items-end gap-1">
                          ...
                          {usagePct != null && (
                            <span className="h-1 w-16 overflow-hidden rounded-pill bg-surface-3" aria-hidden>
                              <span className={cn("block h-full rounded-pill", usagePct >= 90 ? "bg-danger" : usagePct >= 70 ? "bg-warning" : "bg-brand")} style={{ width: `${usagePct}%` }} />
                            </span>
                          )}
                        </div>
                      </TD>
                      ...
                      <TD align="right">
                        <Dropdown menuLabel={`Actions for ${c.code}`} trigger={<IconButton icon={MoreHorizontal} size="sm" label={`Actions for coupon ${c.code}`} />}>
                          {({ close }) => (
                            <>
                              <MenuItem
                                icon={Pencil}
        ...
        <Pagination page={page} totalPages={state.totalPages} total={state.total} pageSize={10} itemLabel={t("couponsPage.itemLabel")} onPageChange={setPage} />
      </Card>

      <CouponDialog open={dialog.open} coupon={dialog.coupon} onClose={() => setDialog({ open: false, coupon: null })} onSaved={load} />

      <ConfirmDialog open={Boolean(confirmDelete)} onClose={() => setConfirmDelete(null)} onConfirm={doDelete} loading={deleting} title={t("confirm.deleteCoupon")} message={confirmDelete ? t("confirm.deleteCouponMsg", { code: confirmDelete.code }) : ""} confirmLabel={t("common.delete")} />
    </div>
  );
}
