  ...
  const submit = async (e) => {
    ...
    if (values.type !== 'free_shipping') {
      if (!values.value) errs.value = t('validation.discountValue');
        ...
        errs.value = t('validation.percentMax');
    }
    ...
    if (values.startsAt && values.expiresAt && values.startsAt > values.expiresAt)
      errs.expiresAt = t('validation.expiresAfter');
    setErrors(errs);
    ...
    >
      <form onSubmit={submit} noValidate className="flex flex-col gap-4 pb-2">
        ...
        <Textarea
          ...
          rows={2}
          placeholder={t('couponsPage.note')}
            ...
            <Table>
              ...
              <THead>
                <tr>
                  ...
                  <TH align="right" className="hidden sm:table-cell">{t('couponsPage.colUsage')}</TH>
                ...
                {state.data.map((c) => {
                  ...
                  return (
                    <TR key={c.id}>
                      ...
                      <TD className="hidden sm:table-cell" muted>{t(`couponsPage.${c.type === 'free_shipping' ? 'freeShipping' : c.type}`)}</TD>
