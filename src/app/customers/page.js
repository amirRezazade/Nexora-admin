...
import { Search, Download, X, Users, Eye, Mail, Ban, MoreHorizontal, UserPlus, Crown, DollarSign } from "lucide-react";
import { cn, currency, dateShort, number, relativeTime } from "@/lib/format";
import { useDebouncedValue } from "@/lib/hooks";
import { downloadCSV } from "@/lib/export";
import { fetchCustomers, setFilter, setSort, setPage, setPageSize, clearFilters } from "@/store/slices/customersSlice";
...
export default function CustomersPage() {
  ...
  const { items, status, filters, sort, page, pageSize, total, totalPages, summary } = useSelector((s) => s.customers);
      ...
      <Card className="flex flex-col">
        <div className="flex flex-wrap items-center gap-2 border-b border-line px-4 py-3 sm:px-5">
          <div className="relative min-w-0 flex-1 sm:max-w-xs">
            <Input type="search" icon={Search} placeholder={t("customersPage.search")} aria-label={t("customersPage.search")} value={searchInput} onChange={(e) => setSearchInput(e.target.value)} className="h-8" />
            {searchInput && (
          ...
          <Select size="sm" aria-label="Filter by status" options={[{ value: "", label: t("filters.allStatuses") }, ...Object.entries(CUSTOMER_STATUS).map(([value]) => ({ value, label: t(`status.${value}`) }))]} value={filters.status[0] || ""} onChange={(e) => dispatch(setFilter({ key: "status", value: e.target.value ? [e.target.value] : [] }))} className="w-36" />
        ...
        {status === "failed" ? (
          ...
          <EmptyState
            ...
            description={hasFilters ? "No customers match your current filters." : "Customers appear here after their first order."}
            action={
              hasFilters && (
                <Button
                  variant="secondary"
            ...
            <Table>
              ...
              <THead>
                <tr>
                  ...
                  <TH align="right" sortable sortKey="orders" currentSort={sort} onSort={(k) => dispatch(setSort(k))}>
                    {t("customersPage.colOrders")}
                  </TH>
                  <TH align="right" sortable sortKey="totalSpent" currentSort={sort} onSort={(k) => dispatch(setSort(k))}>
                    {t("customersPage.colSpent")}
                  </TH>
                  <TH align="right" sortable sortKey="avgOrder" currentSort={sort} onSort={(k) => dispatch(setSort(k))} className="hidden lg:table-cell">
                    {t("customersPage.colAvg")}
                  </TH>
                  <TH sortable sortKey="lastOrder" currentSort={sort} onSort={(k) => dispatch(setSort(k))}>
                    {t("customersPage.colLast")}
                  </TH>
                ...
                {items.map((c) => (
                  <TR key={c.id}>
                    <TD className={stickyLeft}>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <Link href={`/customers/${c.id}`} className="block truncate text-body-sm font-semibold text-ink underline-offset-4 transition-colors hover:text-brand-text hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand">
                            {c.name}
                          </Link>
                    ...
                    <TD align="right" numeric strong>
                      {currency(c.totalSpent, { decimals: 0 })}
                    </TD>
                    <TD align="right" numeric className="hidden lg:table-cell">
                      {currency(c.avgOrder, { decimals: 0 })}
                    </TD>
                    <TD numeric muted>
                      {c.lastOrder ? (
                        <>
        ...
        <Pagination page={page} totalPages={totalPages} total={total} pageSize={pageSize} itemLabel={t("customersPage.itemLabel")} onPageChange={(p) => dispatch(setPage(p))} onPageSizeChange={(s) => dispatch(setPageSize(s))} />
      </Card>
    </div>
  );
}
