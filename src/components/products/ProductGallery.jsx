...
function Art({ seed, url, className, label, name }) {
  ...
  return (
    <span role="img" aria-label={label} className={cn("flex items-center justify-center overflow-hidden", className)} style={{ backgroundImage: `linear-gradient(135deg, ${from}, ${to})` }}>
...
function NavButton({ dir, onClick, label }) {
  ...
  return (
    <button type="button" onClick={onClick} aria-label={label} className={cn("absolute top-1/2 z-10 flex h-9 w-9 -translate-y-1/2 items-center justify-center", "rounded-full border border-white/20 bg-black/45 text-white shadow-md backdrop-blur-sm", "transition-colors hover:bg-black/65 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand", dir === "prev" ? "start-3" : "end-3")}>
...
export default function ProductGallery({ productId, name, images: incoming, editable = false, onChange }) {
  ...
  const fileInput = <input ref={fileRef} type="file" accept="image/*" multiple className="sr-only" onChange={onFiles} />;
  ...
  return (
    <div className="nexora-gallery flex flex-col gap-3">
      ...
      <div className="group relative aspect-square w-full overflow-hidden rounded-card-lg border border-line bg-surface-2">
        ...
        >
          {images.map((img, i) => (
            <SwiperSlide key={img.id}>
              <Art seed={img.seed} url={img.url} name={name} label={`${name}, image ${i + 1} of ${images.length}`} className="h-full w-full" />
            </SwiperSlide>
          ))}
        </Swiper>
        ...
        <div className="pointer-events-none absolute end-3 top-3 z-10 flex gap-1.5 opacity-0 transition-opacity duration-150 group-hover:opacity-100 group-focus-within:opacity-100">
          <span className="pointer-events-auto">
            <IconButton icon={Maximize2} label={t("gallery.preview")} variant="secondary" size="sm" onClick={() => setPreview(true)} />
          </span>
          {editable && images.length > 0 && (
            <span className="pointer-events-auto">
              <IconButton icon={Trash2} label={t("gallery.remove")} variant="secondary" size="sm" className="text-danger-text" onClick={() => remove(active)} />
            </span>
          )}
        </div>
        {active === 0 && (
      ...
      <Swiper modules={[Thumbs, A11y]} onSwiper={setThumbsSwiper} watchSlidesProgress slidesPerView={5} spaceBetween={8} className="nexora-gallery-thumbs w-full">
        {images.map((img, i) => (
          <SwiperSlide key={img.id} className="!h-auto !w-auto max-w-none flex-[1_0_calc(20%-6.4px)]">
            <button type="button" aria-label={`${name} ${i + 1}`} aria-current={active === i} onClick={() => mainSwiper?.slideTo(i)} className={cn("relative aspect-square w-full overflow-hidden rounded-control border-2 transition-all duration-150", "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 focus-visible:ring-offset-canvas", active === i ? "border-brand" : "border-line hover:border-line-strong")}>
      ...
      {editable && <p className="text-caption text-ink-3">{t("gallery.hint")}</p>}

      <Modal open={preview} onClose={() => setPreview(false)} title={name} size="lg">
        <div className="pb-4">{current && <Art seed={current.seed} url={current.url} name={name} label={name} className="aspect-square w-full rounded-card-lg" />}</div>
      </Modal>
    </div>
  );
}
