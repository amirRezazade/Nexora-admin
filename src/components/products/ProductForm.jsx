...
function normalizeProduct(initial) {
  ...
  TEXT_KEYS.forEach((k) => {
    if (merged[k] == null) merged[k] = EMPTY[k];
  });
...
function FormSection({ isMobile, id, title, description, icon, children, defaultOpen = false }) {
  return isMobile ? (
...
export default function ProductForm({ mode = "create", initial = null, productId }) {
  ...
  useEffect(() => {
    if (initial) setValues(normalizeProduct(initial));
  }, [initial]);
