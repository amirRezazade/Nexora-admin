...
import {
  LayoutDashboard, LineChart, Package, FolderTree, Boxes, ShoppingCart, Users,
  Ticket, Star, Bell, Settings, LifeBuoy, PanelLeftClose, PanelLeft, ChevronRight,
} from 'lucide-react';
...
export function NovaMark({ className }) {
  return (
    <span
      aria-hidden
      className={cn(
        'flex h-8 w-8 shrink-0 items-center justify-center rounded-[9px] bg-brand text-white shadow-xs',
        className
      )}
    >
...
function NavLink({ item, collapsed, unread, onNavigate }) {
  ...
  const link = (
    <Link
      ...
      className={cn(
        'group relative flex items-center gap-2.5 rounded-control py-2 text-body-sm font-medium transition-colors duration-150',
    ...
    >
      ...
      {active && (
        <span
          aria-hidden
          className={cn(
            'absolute rounded-pill bg-brand',
            collapsed ? '-start-2 top-1/2 h-5 w-1 -translate-y-1/2' : '-start-3 top-1/2 h-5 w-[3px] -translate-y-1/2'
          )}
        />
      )}
      ...
      {!collapsed && badge && (
        <span className="rounded-pill bg-brand px-1.5 text-[11px] font-semibold leading-4 tabular-nums text-white">
          {badge > 99 ? '99+' : badge}
        </span>
      )}
      {collapsed && badge && (
        <span aria-hidden className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-brand ring-2 ring-surface" />
      )}
    </Link>
  );
    ...
    <div className="flex h-full flex-col bg-surface">
      ...
      <div className={cn('flex h-14 shrink-0 items-center border-b border-line', collapsed ? 'justify-center px-2' : 'gap-2.5 px-4')}>
        <Link
          href="/"
          onClick={onNavigate}
          className="flex min-w-0 items-center gap-2.5 rounded-control focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
      ...
      <nav aria-label="Main navigation" className={cn('flex-1 overflow-y-auto overflow-x-hidden py-3', collapsed ? 'px-3' : 'px-3')}>
        {NAV_GROUPS.map((group, gi) => (
          <div key={group.labelKey} className={cn(gi > 0 && 'mt-5')}>
            {collapsed ? (
              gi > 0 && <div aria-hidden className="mx-auto mb-3 h-px w-6 bg-line" />
            ) : (
              <p className="mb-1.5 px-2.5 text-micro uppercase tracking-wider text-ink-3">{t(group.labelKey)}</p>
            )}
      ...
      <div className={cn('shrink-0 border-t border-line py-3', collapsed ? 'px-3' : 'px-3')}>
        <ul className="flex flex-col gap-0.5">
          <li>
            <NavLink
              item={{ href: '/support', labelKey: 'nav.support', icon: LifeBuoy }}
              collapsed={collapsed}
        ...
        <Link
          ...
          onClick={onNavigate}
          className={cn(
            'mt-2 flex items-center gap-2.5 rounded-control py-2 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand',
            collapsed ? 'justify-center px-0' : 'px-2',
            isActive(pathname, '/profile') ? 'bg-brand-soft' : 'hover:bg-surface-3'
          )}
        >
        ...
        <button
          ...
          className={cn(
            'mt-2 hidden w-full items-center gap-2.5 rounded-control py-2 text-body-sm font-medium text-ink-3 transition-colors hover:bg-surface-3 hover:text-ink lg:flex',
...
export default function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        'fixed inset-y-0 start-0 z-40 hidden shrink-0 border-e border-line transition-[width] duration-200 ease-inout lg:block',
  ...
  return (
    <div className="fixed inset-0 z-[60] lg:hidden">
      ...
      <div
        ...
        className="absolute inset-y-0 left-0 w-[272px] animate-slide-in-left border-r border-line shadow-xl"
      >
