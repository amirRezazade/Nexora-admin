...
import {
  Menu, Search, Bell, Sun, Moon, Monitor, LogOut, User, Settings as SettingsIcon,
  CheckCheck, Package, ShoppingCart, Star, Server, ChevronDown,
} from 'lucide-react';
  ...
  return (
    <Dropdown
      ...
      trigger={
        <button
          type="button"
          className="hidden h-8 min-w-[40px] items-center justify-center rounded-control px-2 text-caption font-semibold text-ink-2 transition-colors hover:bg-surface-3 hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand sm:inline-flex"
      ...
      trigger={
        <span className="relative inline-flex">
          ...
          {unread > 0 && (
            <span
              aria-hidden
              className="pointer-events-none absolute -right-0.5 -top-0.5 flex h-[15px] min-w-[15px] items-center justify-center rounded-pill bg-brand px-1 text-[9px] font-bold leading-none text-white ring-2 ring-canvas"
            >
              {unread > 9 ? '9+' : unread}
            </span>
          )}
        ...
        <div className="flex flex-col">
          <div className="flex items-center justify-between border-b border-line px-4 py-3">
            <div className="flex items-baseline gap-2">
              ...
              {unread > 0 && <span className="text-caption text-ink-3">{t('header.unread', { n: unread })}</span>}
            </div>
            {unread > 0 && (
              <Button variant="link" size="xs" icon={CheckCheck} onClick={() => dispatch(markAllRead())}>
                  ...
                  return (
                    <li key={n.id}>
                      <Link
                        ...
                        className={cn(
                          'flex gap-3 border-b border-line px-4 py-3 transition-colors last:border-0 hover:bg-surface-2',
                      ...
                      >
                        ...
                        <span className="min-w-0 flex-1">
                          ...
                          <span className="mt-0.5 block text-caption leading-relaxed text-ink-2">{n.body}</span>
          ...
          <div className="border-t border-line bg-surface-2 px-4 py-2.5">
            <Link
              href="/notifications"
              onClick={close}
              className="text-body-sm font-medium text-brand-text underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
            >
              {t('header.viewAll')}
            </Link>
          </div>
...
function ThemeToggle() {
  ...
  return (
    <Dropdown
      width="w-44"
      menuLabel="Theme"
      trigger={<IconButton icon={Icon} label={`${t('header.theme')}: ${preference}`} />}
    >
      {({ close }) => (
        <>
  ...
  return (
    <Dropdown
      ...
      trigger={
        <button
          type="button"
          className="flex items-center gap-1.5 rounded-pill p-0.5 pr-1.5 transition-colors hover:bg-surface-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 focus-visible:ring-offset-canvas"
