    ...
    <div className="flex flex-col items-center gap-5 sm:flex-row">
      <div style={{ height, width: height }} className="relative shrink-0">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={rows} dataKey="revenue" nameKey="label" innerRadius="62%" outerRadius="94%" paddingAngle={2} stroke={c.surface} strokeWidth={2}>
              {rows.map((entry, i) => (
        ...
        {rows.map((d, i) => {
          ...
          return (
            <li key={d.categoryId} className="flex items-center gap-2.5">
              <span aria-hidden className="h-2.5 w-2.5 shrink-0 rounded-[3px]" style={{ backgroundColor: palette[i % palette.length] }} />
              <span className="min-w-0 flex-1 truncate text-body-sm text-ink">{d.label}</span>
              <span className="shrink-0 text-body-sm font-medium tabular-nums text-ink">{currency(d.revenue, { decimals: 0 })}</span>
              <span className="w-11 shrink-0 text-right text-caption tabular-nums text-ink-3">{share.toFixed(1)}%</span>
            </li>
          );
        })}
      </ul>
