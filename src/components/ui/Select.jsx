...
const Select = forwardRef(function Select(
  { label, hint, error, required, options = [], placeholder, className, size = 'md', id, children, ...props },
  ref
) {
  const autoId = useId();
      ...
      <div className="relative">
        <select
          ...
          className={cn(
            ...
            size === 'sm' ? 'h-8 ps-2.5 text-body-sm' : 'h-9 ps-3 text-body',
        ...
        <ChevronDown
          aria-hidden
          className="pointer-events-none absolute end-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-3"
        />
      </div>
    </FieldShell>
  );
