  ...
  return (
    <FieldShell label={label} htmlFor={inputId} required={required} hint={hint} error={error} className={wrapperClassName} action={action}>
      <div className="relative flex items-center">
        {Icon && <Icon aria-hidden className="pointer-events-none absolute left-3 h-4 w-4 text-ink-3" />}
        {prefix && <span className="pointer-events-none absolute left-3 text-body text-ink-3">{prefix}</span>}
        <input
          ref={ref}
          id={inputId}
          type={type}
          ...
          className={cn(fieldBase, "h-9 px-3 text-body", Icon && "pl-9", prefix && "pl-7", suffix && "pr-10", error ? "border-danger focus:border-danger focus:ring-danger/20" : "border-line-strong", className)}
          {...props}
        />
        {suffix && <span className="pointer-events-none absolute right-3 text-caption text-ink-3">{suffix}</span>}
      </div>
      {(error || hint) && (
