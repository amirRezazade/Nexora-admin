...
export default function Modal({ open, onClose, title, description, children, footer, size = "md", closeOnOverlay = true }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[70] flex items-end justify-center p-0 sm:items-center sm:p-6">
      <div className="absolute inset-0 animate-fade-in bg-overlay/45 backdrop-blur-[2px]" onClick={closeOnOverlay ? onClose : undefined} aria-hidden />
      <div ref={panelRef} role="dialog" aria-modal="true" aria-labelledby="modal-title" aria-describedby={description ? "modal-description" : undefined} className={cn("relative max-h-[calc(100dvh-1.5rem)] w-full animate-slide-up overflow-y-auto rounded-t-container border border-line bg-surface shadow-xl sm:rounded-card-lg", widths[size])}>
        ...
        {footer && <div className="mt-2 flex flex-col-reverse gap-2 border-t border-line px-5 py-4 sm:flex-row sm:justify-end sm:px-6">{footer}</div>}
      </div>
    </div>,
    document.body,
  );
...
export function ConfirmDialog({ open, onClose, onConfirm, title, message, confirmLabel, cancelLabel, loading = false, tone = "danger" }) {
  const { t } = useI18n();
  return (
    <Modal
      open={open}
      ...
      footer={
        <>
          ...
          <Button variant={tone === "danger" ? "danger" : "primary"} onClick={onConfirm} loading={loading} data-autofocus className="sm:w-auto">
