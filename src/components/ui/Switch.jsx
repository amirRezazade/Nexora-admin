...
export default function Switch({ checked, onChange, label, description, disabled, className, id }) {
  ...
  return (
    <div className={cn("flex items-start justify-between gap-4", className)}>
      ...
      >
        <span aria-hidden className={cn("pointer-events-none absolute top-px inline-block h-4 w-4 rounded-full bg-white shadow-sm transition-[inset-inline-start] duration-200 ease-out", checked ? "start-[18px]" : "start-0.5")} />
      </button>
    </div>
  );
}
