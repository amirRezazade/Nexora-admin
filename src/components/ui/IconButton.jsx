...
const IconButton = forwardRef(function IconButton(
  { icon: Icon, label, variant = 'ghost', size = 'md', className, active, loading = false, disabled, ...props },
  ref
) {
  ...
  return (
    ...
    >
      {loading ? (
        <Loader2 aria-hidden className={cn(iconSizes[size], 'animate-[spin_0.7s_linear_infinite]')} />
      ) : (
        <Icon aria-hidden className={iconSizes[size]} />
      )}
    </button>
  );
});
