...
export default function Dropdown({ trigger, children, align = "end", width = "w-56", className, menuLabel }) {
  ...
  const menu =
    open && typeof document !== "undefined"
      ? createPortal(
          <div ref={menuRef} role="menu" aria-label={menuLabel} onKeyDown={onKeyDown} style={{ position: "fixed", top: coords.top, left: coords.left, transformOrigin: coords.transformOrigin }} className={cn("z-[80] max-h-[min(320px,calc(100vh-16px))] overflow-y-auto rounded-card border border-line bg-surface p-1 shadow-lg animate-pop-in", width, className)}>
...
export function MenuItem({ icon: Icon, children, onClick, destructive, disabled, shortcut, className, as: Tag = "button", ...props }) {
  return (
    <Tag role="menuitem" tabIndex={-1} disabled={Tag === "button" ? disabled : undefined} onClick={onClick} className={cn("flex w-full items-center gap-2.5 rounded-[7px] px-2.5 py-1.5 text-start text-body-sm transition-colors", "focus:outline-none focus-visible:bg-surface-3", destructive ? "text-danger-text hover:bg-danger-soft focus-visible:bg-danger-soft" : "text-ink-2 hover:bg-surface-3 hover:text-ink", disabled && "pointer-events-none opacity-45", className)} {...props}>
...
export function MenuLabel({ children }) {
  return <div className="px-2.5 pb-1 pt-2 text-micro uppercase tracking-wide text-ink-3">{children}</div>;
}
